...
template <int ROWS,
          int THREADS_PER_ROW,
          typename elem_type = __half,
          int M,
          typename Gmem_softmax_sum>
inline __device__ void dot_do_o(const uint4 (&do_)[M],
                                const uint4 (&o)[M],
                                const float scale,
...
#pragma unroll
  ...
  if ((dp_sum_row < ROWS) && (tidx % THREADS_PER_ROW == 0)) {
    gmem_softmax_d.store_row(reinterpret_cast<const uint32_t(&)[M]>(sum),
                             dp_sum_row);
  }
}
...
inline __device__ void compute_dot_do_o(const Params &params) {
...
#else
  constexpr bool is_fp16_type =
      std::is_same<typename Kernel_traits::elem_type, __half>::value;
  assert(is_fp16_type);
  ...
  using Cta_tile_dkv = fmha::Cta_tile_extd<Cta_tile_p::N,
                                           Cta_tile_p::K,
                                           Cta_tile_p::M,
                                           Cta_tile_p::WARPS_N,
                                           1,
  ...
  static_assert(Cta_tile_dkv::N == 16 || Cta_tile_dkv::N == 32 ||
                Cta_tile_dkv::N == 64 || Cta_tile_dkv::N == 128);
  ...
  Gmem_tile_do gmem_do(params.do_ptr,
  ...
  Gmem_tile_o gmem_o(params.o_ptr,
                     params.o_row_stride_in_elts,
                     params.o_head_stride_in_elts,
                     params.d,
                     binfo,
  ...
  for (int l = blockIdx.z; l < steps; l += step_stride) {
    if (l * Cta_tile_p::M >= binfo.actual_seqlen_q) break;
...
template <typename Kernel_traits,
          ...
          typename Params,
          typename Prng>
inline __device__ void compute_dq_dk_dv_1xN_one_iter(const Params &params,
                                                     Prng &ph,
  ...
  static_assert(Cta_tile_dkv::M == 512 || Cta_tile_dkv::M == 256 ||
                Cta_tile_dkv::M == 128);
  static_assert(Cta_tile_dkv::N == 16 || Cta_tile_dkv::N == 32 ||
                Cta_tile_dkv::N == 64 || Cta_tile_dkv::N == 128);
  static_assert(Cta_tile_dkv::K == 16);
  ...
  using Smem_tile_qt =
      fmha::Smem_tile_b<Cta_tile_dkv, fmha::Row, Gmem_tile_q::BYTES_PER_LDG, 2>;
  ...
  // Treating V as K. We need to use Kernel_traits::Smem_tile_k otherwise
  // loading will be wrong The global memory tile to load V.
  ...
  Gmem_tile_q gmem_q(params.q_ptr,
  ...
  Gmem_tile_dq gmem_dq(params.dq_ptr,
                       params.dq_row_stride_in_elts,
                       params.dq_head_stride_in_elts,
                       params.d,
                       binfo,
                       tidx);
  Gmem_tile_dq_tmp gmem_dq_tmp(params.o_tmp_ptr,
                               params.o_row_stride_in_elts,
                               params.o_head_stride_in_elts,
                               params.d,
                               binfo,
  ...
  Gmem_tile_k gmem_k(params.k_ptr,
  ...
  Gmem_tile_v gmem_v(params.v_ptr,
  ...
  // Allocate the shared memory tile loader for K^T. We use the same as K so be
  // careful!!!
  Smem_tile_kt smem_kt(
      &smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::Smem_tile_q::BYTES_PER_TILE],
      tidx);
  ...
  Smem_tile_st smem_s(
      &smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::SMEM_OFFSET_O +
             Smem_tile_dq::BYTES_PER_TILE],
      tidx);
  ...
  // careful!!!
  Smem_tile_dq smem_dq(
      &smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::SMEM_OFFSET_O], tidx);
...
#pragma unroll
    for (int i = 0; i < Smem_tile_dk::NUM_LDS; ++i) {
      dkv_out[i] = make_uint4(0u, 0u, 0u, 0u);
    }
    Gmem_tile_dk gmem_dk(params.dk_ptr,
                         params.dk_row_stride_in_elts,
                         params.dk_head_stride_in_elts,
                         params.d,
                         binfo,
                         tidx,
                         false);
    ...
    Gmem_tile_dv gmem_dv(params.dv_ptr,
                         params.dv_row_stride_in_elts,
                         params.dv_head_stride_in_elts,
                         params.d,
                         binfo,
                         tidx,
                         false);
  ...
  const int steps =
      (params.seqlen_q + Cta_tile_p::M - 1) / Cta_tile_p::M - begin;
  // Wind gmem tiles to the correct position.
  gmem_q.move(begin);
  gmem_do.move(begin);
  gmem_o.move(begin);
  ...
  gmem_softmax_lse.load(
      reinterpret_cast<uint32_t(&)[Mma_tile_p::MMAS_M * 2]>(p_lse));
  ...
  // kernel.
  typename Smem_tile_v::Fragment
      frag_v[Kernel_traits::V_IN_REGS ? Mma_tile_p::MMAS_K : 2]
            [Mma_tile_p::MMAS_N];
...
#pragma unroll
  ...
  gmem_softmax_d.load(
      reinterpret_cast<uint32_t(&)[Mma_tile_p::MMAS_M * 2]>(dp_sum));
  ...
  // frag_kt[Mma_tile_dq::MMAS_K][Mma_tile_dq::MMAS_N]; #pragma unroll for( int
  // ki = 0; ki < Mma_tile_dq::MMAS_K; ++ki ) {
  ...
  fmha::Clear_accumulator<fmha::Accumulator_type, Cta_tile_dkv::WARPS_K>::apply(
      acc_dv);
  ...
  for (int l = 0; l < steps; l++) {
    ...
    softmax.template scale_apply_exp</*scale_max=*/false>(p_lse,
                                                          params.scale_bmm1f);
    if (Is_dropout) {
      ...
      // apply_dropout_16bits</*encode_dropout_in_sign_bit=*/true>(ph,
      // params.p_dropout_in_uint16_t);
      unsigned int warp_idx = threadIdx.x / 32;
      // TODO: this should change after we rearrange the warps (e.g. cutlass
      // branch)
      unsigned int block_col_idx =
          loop_step_idx * Cta_tile_p::N / 16 + warp_idx;
      unsigned long long philox_subsequence =
          (begin + l) * (binfo.actual_seqlen_k / 16) + block_col_idx;
      softmax
          .template apply_dropout_16bits</*encode_dropout_in_sign_bit=*/true>(
              ph, params.p_dropout_in_uint16_t, philox_subsequence);
    }
...
#pragma unroll
    for (int ki = 1; ki < Mma_tile_p::MMAS_K; ++ki) {
      ...
      if (!Kernel_traits::V_IN_REGS) {
        ...
        fmha::gemm_cl<elem_type>(
            acc_dp, frag_do[(ki - 1) & 1], frag_v[(ki - 1) & 1]);
      } else {
        fmha::gemm_cl<elem_type>(acc_dp, frag_do[(ki - 1) & 1], frag_v[ki - 1]);
      }
      // if ((threadIdx.x == 0) && (blockIdx.x == 0) && (blockIdx.y == 0) && (l
      // < 4))  {
      //     float2 tmp = __half22float2(reinterpret_cast<__half2 &>(frag_do[(ki
      //     - 1) & 1])); printf("frag_do=%.6f, %.6f\n", tmp.x, tmp.y); tmp =
      //     __half22float2(reinterpret_cast<__half2 &>(frag_v[(ki - 1) & 1]));
      //     printf("frag_v=%.6f, %.6f\n", tmp.x, tmp.y);
      // }
    }
    ...
    // Do the final stage of math.
    {
      ...
      if (!Kernel_traits::V_IN_REGS) {
        ...
        fmha::gemm_cl<elem_type>(
            acc_dp, frag_do[(ki - 1) & 1], frag_v[(ki - 1)]);
      }
    }
...
#pragma unroll
      for (int ni = 0; ni < Mma_tile_p::MMAS_N; ni++) {
        softmax.elt_[2 * mi + 0][4 * ni + 0] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 0],
                           acc_dp[mi][ni].elt(0),
                           dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 1] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 1],
                           acc_dp[mi][ni].elt(1),
                           dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 2] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 2],
                           acc_dp[mi][ni].elt(4),
                           dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 0][4 * ni + 3] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 3],
                           acc_dp[mi][ni].elt(5),
                           dp_sum[2 * mi + 0]);
        softmax.elt_[2 * mi + 1][4 * ni + 0] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 0],
                           acc_dp[mi][ni].elt(2),
                           dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 1] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 1],
                           acc_dp[mi][ni].elt(3),
                           dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 2] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 2],
                           acc_dp[mi][ni].elt(6),
                           dp_sum[2 * mi + 1]);
        softmax.elt_[2 * mi + 1][4 * ni + 3] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 3],
                           acc_dp[mi][ni].elt(7),
                           dp_sum[2 * mi + 1]);
      }
    }
    ...
    fmha::Clear_accumulator<typename fmha::Accumulator_type,
                            Cta_tile_dq::WARPS_K>::apply(acc_dq);
...
#pragma unroll
    ...
    // Need to syncthreads here, otherwise the smem_dq reads from the previous
    // iteration might happen after the smem_dq writes in this iteration.
    __syncthreads();
    ...
    __syncwarp();
    typename Smem_tile_st::Fragment frag_s[Mma_tile_dkv::MMAS_K]
                                          [Mma_tile_dkv::MMAS_M];
    smem_s.load(frag_s);
...
#pragma unroll
    for (int ki = 1; ki < Mma_tile_dkv::MMAS_K; ++ki) {
      ...
      // Do the math for the values already in registers.
      fmha::gemm_cl<elem_type>(
          acc_dv, frag_s[(ki - 1)], frag_dot[(ki - 1) & 1]);
    }
    ...
    //     float2 tmp0 = __half22float2(reinterpret_cast<__half2
    //     &>(frag_dot[0][0])); printf("frag_dot[0][0]=%.6f, %.6f\n", tmp0.x,
    //     tmp0.y); float2 tmp1 = __half22float2(reinterpret_cast<__half2
    //     &>(frag_dot[0][1])); printf("frag_dot[0][1]=%.6f, %.6f\n", tmp1.x,
    //     tmp1.y);
    // }

    // if ((threadIdx.x == 0) && (blockIdx.x == 0) && (blockIdx.y == 0))  {
    //     printf("l = %d, acc_dv[0][0]=%.6f, %.6f\n", l, acc_dv[0][0].elt(2),
    //     acc_dv[0][0].elt(3)); printf("l = %d, acc_dv[0][1]=%.6f, %.6f\n", l,
    ...
    typename Smem_tile_st::Fragment frag_dpt[Mma_tile_dkv::MMAS_K]
                                            [Mma_tile_dkv::MMAS_M];
...
#pragma unroll
    for (int ki = 1; ki < Mma_tile_dkv::MMAS_K; ++ki) {
      ...
      // Do the math for the values already in registers.
      fmha::gemm_cl<elem_type>(
          acc_dk, frag_dpt[(ki - 1)], frag_qt[(ki - 1) & 1]);
    }
    ...
    if (!Seq_parallel) {
      ...
      // We always scale dq_out before writing in this case, since we don't want
      // to have to scale at the end when copying from dq_tmp to dq.
      for (int jj = 0; jj < Gmem_tile_dq::STGS_PER_LOOP; ++jj) {
        // dq_out[jj] = fmha::fmul4(dq_out[jj], params.scale_bmm1f);
        dq_out[jj] = fmha::fmul4(dq_out[jj], params.scale_bmm1_rp_dropout);
      }
  ...
  //     acc_dv[0][0].elt(3)); printf("l final, acc_dv[0][1]=%.6f, %.6f\n",
  //     acc_dv[0][1].elt(2), acc_dv[0][1].elt(3));
  // }
  for (int mi = 0; mi < Mma_tile_dkv::MMAS_M; mi++) {
    for (int ni = 0; ni < Mma_tile_dkv::MMAS_N; ni++) {
  ...
  __syncthreads();
  // TODO [TD - 2022-05-04]: Are there cases where the shared mem for dV and dK
  // are larger than the total amount of shared mem? Epilogue swizzle for dV
...
// Kernel_traits::Cta_tile_p::N. This template parameter is there so we can
// specialize with loop_steps == 1 and loop_steps == 2.
template <typename Kernel_traits,
          bool Is_dropout,
          bool Is_causal,
          int loop_steps = -1,
...
inline __device__ void compute_dq_dk_dv_1xN(const Params &params) {
  ...
  if (loop_steps == 1) {
    compute_dq_dk_dv_1xN_one_iter<Kernel_traits,
                                  Is_dropout,
                                  Is_causal,
                                  true,
                                  true>(params, ph, 0);
    ...
    if (params.seqlen_k == blocksize_c) {
      ...
      const int max_loop_steps =
          (params.seqlen_k + blocksize_c - 1) / blocksize_c;
      compute_dq_dk_dv_1xN_one_iter<Kernel_traits,
                                    Is_dropout,
                                    Is_causal,
                                    true,
      ...
      for (int loop_step_idx = 1; loop_step_idx < max_loop_steps - 1;
        ...
        compute_dq_dk_dv_1xN_one_iter<Kernel_traits,
                                      ...
                                      Is_causal,
                                      false,
                                      false>(params, ph, loop_step_idx);
      }
      compute_dq_dk_dv_1xN_one_iter<Kernel_traits,
                                    Is_dropout,
                                    Is_causal,
                                    false,
                                    true>(params, ph, max_loop_steps - 1);
    }
  }
}
