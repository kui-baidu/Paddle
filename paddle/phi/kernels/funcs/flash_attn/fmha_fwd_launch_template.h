...
// 95% of the best efficiency. [2022-11-25] TD: Mark this as "inline" otherwise
// we get "multiple definition" error.
inline int num_splits_heuristic_fwd(int batch_nheads,
                                    int num_SMs,
                                    int ctas_per_sm,
                                    int max_splits) {
...
void run_fmha_fwd_loop(Launch_params<FMHA_fprop_params> &launch_params) {
  ...
  const int loop_steps =
      (launch_params.params.seqlen_k + blocksize_c - 1) / blocksize_c;
      ...
      launch_params.is_dropout, IsDropoutConst, ({
        auto kernel = launch_params.params.is_causal
                          ? (launch_params.return_softmax
                                 ? &fmha_fwd_loop_kernel<Kernel_traits,
                                                         IsDropoutConst,
                                                         true,
                                                         true>
                                 : &fmha_fwd_loop_kernel<Kernel_traits,
        ...
        if (launch_params.params.num_splits <= 0) {
          ...
          launch_params.params.num_splits = num_splits_heuristic_fwd(
              ...
              ctas_per_sm,
              /*max_splits=*/
              std::min(30, (launch_params.params.seqlen_q + M - 1 / M)));
        }
        // printf("smem_size = %d\n", smem_size);
        dim3 grid(launch_params.params.b,
                  launch_params.params.h,
                  launch_params.params.num_splits);
        kernel<<<grid,
                 Kernel_traits::THREADS,
                 smem_size,
                 launch_params.stream>>>(launch_params.params);
