...
template <typename Kernel_traits,
          ...
          bool Is_causal,
          int loop_steps = -1>
__global__ void fmha_block_dgrad_fp16_sm80_dq_dk_dv_loop_kernel(
    FMHA_dgrad_params params) {
  fmha::compute_block_dq_dk_dv_1xN<Kernel_traits,
                                   Is_dropout,
                                   Is_causal,
                                   loop_steps>(params);
}
...
template <typename Kernel_traits>
void run_fmha_block_dgrad_fp16_sm80_loop_(const FMHA_dgrad_params &params,
                                          cudaStream_t stream) {
  ...
  constexpr int smem_size_dq_dk_dv =
      ...
      smem_size_dq + smem_size_s * 2 + smem_size_dp_sum;
...
void run_fmha_block_dgrad_fp16_sm80(const FMHA_dgrad_params &params,
                                    cudaStream_t stream) {
  if (params.d == 16) {
