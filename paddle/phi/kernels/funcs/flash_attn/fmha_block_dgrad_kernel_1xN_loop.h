...
inline __device__ void dot_do_o(float (&sum)[M],
                                const uint4 (&do_)[M],
                                const uint4 (&o)[M],
                                Smem_dp_sum smem,
...
inline __device__ void compute_block_dq_dk_dv_1xN_one_iter(
    const Params &params, Prng &ph, const int loop_step_idx) {
  ...
  Smem_dp_sum smem_dp_sum(
      reinterpret_cast<float *>(
          &smem_[Smem_tile_do::BYTES_PER_TILE + Gemm1::SMEM_OFFSET_O +
                 Smem_tile_dq::BYTES_PER_TILE +
                 Smem_tile_st::BYTES_PER_TILE * 2]),
      tidx);
  ...
  if (Is_first || mask_val % 2 == 1) {
    ...
    if ((dp_sum_row < Smem_dp_sum::ROWS) &&
        (tidx % Smem_dp_sum::THREADS_PER_ROW == 0)) {
      gmem_softmax_d.store_row(
          reinterpret_cast<uint32_t(&)[Gmem_tile_do::LDGS]>(dp_sum_regs),
          dp_sum_row);
    }
  }
...
#pragma unroll
  ...
  for (int l = 0; l < steps; l++) {
    ...
    // TD [2022-04-24]: if Is_first, then it's faster to set acc_dp to zero then
    // subtract by dp_sum later. If !Is_first, then it's faster to set acc_dp to
    // -dp_sum and don't subtract later. This is because loading dp_sum earlier
    // uses more registers.
    fmha::Fragment_accumulator acc_dp[Mma_tile_p::MMAS_M][Mma_tile_p::MMAS_N];
    // if (Is_first) {
    // if (true) {
    if (is_first_read) {
      fmha::Clear_accumulator<fmha::Accumulator_type,
                              Cta_tile_p::WARPS_K>::apply(acc_dp);
    } else {
#pragma unroll
...
#pragma unroll
    ...
    // // TD [2022-04-01]: Don't need to apply mask since the corresponding
    // value in softmax
    // // will be zero.
    // for (int mi = 0; mi < Mma_tile_p::MMAS_M * 2; mi++) { dp_sum[mi] *=
    // params.p_dropout; } if (Is_first) { softmax.subtract_dp_sum(dp_sum); } if
    // (true) { softmax.subtract_dp_sum(dp_sum); }
    if (is_first_read) {
      softmax.subtract_dp_sum(dp_sum);
    }
...
#pragma unroll
          for (int ii = 0; ii < 4; ++ii) {
            ...
            const __half2 pdp =
                __hmul2(p, frag_dp[mi][ni].template elt_as<__half2>(ii));
            ...
            const __half low =
                __low2half(p) >= zero_h ? __low2half(pdp) : __low2half(pd);
            const __half high =
                __high2half(p) >= zero_h ? __high2half(pdp) : __high2half(pd);
            frag_p[mi][ni].template elt_as<__half2>(ii) =
...
#pragma unroll
    ...
    is_first_read ? smem_dq.template load</*zero_init=*/true>(dq_out)
                  : smem_dq.template load</*zero_init=*/false>(dq_out);
...
inline __device__ void compute_block_dq_dk_dv_1xN(const Params &params) {
  ...
  if (loop_steps == 1) {
    compute_block_dq_dk_dv_1xN_one_iter<Kernel_traits,
                                        Is_dropout,
                                        Is_causal,
                                        true,
                                        true>(params, ph, 0);
